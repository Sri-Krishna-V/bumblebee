...
from bumblebee.config import env_field, strict_kwargs
from bumblebee.models import DocumentInput
from bumblebee.stats import build_run_summary
...
class BatchPolicy:
    ...
    Fields fall back to their ``BUMBLEBEE_*`` environment variables like every
    other config (explicit values win).
    """

    max_docs: int = env_field("BUMBLEBEE_BATCH_MAX_DOCS", 64, int)
    max_bytes: int = env_field("BUMBLEBEE_BATCH_MAX_BYTES", 512 * 1024 * 1024, int)
    max_pages: int | None = env_field("BUMBLEBEE_BATCH_MAX_PAGES", None, int)
    retries: int = env_field("BUMBLEBEE_BATCH_RETRIES", 3, int)
    retry_backoff_seconds: float = env_field("BUMBLEBEE_BATCH_RETRY_BACKOFF_SECONDS", 10.0, float)
...
class RunTargetOps(Protocol):
    """Target-side operations a supervisor needs (see :class:`bumblebee.runs.RunTarget`)."""
