...
_PACKAGE_LOGGER = "bumblebee"
_DEFAULT_LEVEL = "INFO"
_LOG_FORMAT = "%(asctime)s %(levelname)s %(name)s %(message)s"


def configure_logging(level: str | None = None) -> None:
    """Attach a stderr handler to the ``bumblebee`` logger.

    Idempotent: safe to call from every entrypoint. The level defaults to the
    ``BUMBLEBEE_LOG_LEVEL`` environment variable, then ``INFO``.

    Args:
        level: Explicit level name (e.g. ``"DEBUG"``) overriding the environment.
    """
    resolved = (level or os.environ.get("BUMBLEBEE_LOG_LEVEL") or _DEFAULT_LEVEL).upper()
