...
(no provider SDK). SSH into a GPU instance, install the package, and run::
    ...
    bumblebee --source ./input --target ./output
...
to ``modal run`` (install the ``bumblebee[modal]`` extra)::

    bumblebee modal --source ./input --target ./output           # batched Modal GPU workflow
    bumblebee modal --source s3://in --target s3://out --detach   # cloud run detached in Modal

Local paths and cloud URIs (``s3://``, ``gs://``, ``az://``) both work; install
the matching storage extra (``bumblebee[s3]`` etc.) for cloud URIs.

Every settings flag is optional: an unset flag falls back to its ``BUMBLEBEE_*``
...
from bumblebee.config import EngineConfig, LayoutBackend, ModalConfig, OcrConfig, env_overrides
...
def run(
    ...
    # Imported here (not at module load) so `bumblebee --help` works without the
...
def modal(
    ...
    batch_retries: BatchRetriesOpt = None,
    batch_retry_backoff_seconds: BatchRetryBackoffOpt = None,
    app_name: Annotated[str | None, typer.Option(help="Modal app name. [default: bumblebee]")] = None,
    ...
    if max_inflight_pdfs is None and not os.environ.get("BUMBLEBEE_MAX_INFLIGHT_PDFS"):
    ...
    if log_level is not None:
        env["BUMBLEBEE_LOG_LEVEL"] = log_level
