...
from bumblebee.config import EngineConfig, OcrConfig
from bumblebee.layout import LayoutEngine
from bumblebee.models import DocumentInput, DocumentResult, OcrError, ProcessedDoc
from bumblebee.ocr import VllmOcrClient
from bumblebee.ocr.server import base_url, start_vllm_server, stop_vllm_server, wait_for_health
from bumblebee.pdf import RenderEngine
from bumblebee.pipeline import Pipeline
from bumblebee.runs import (
    RunTarget,
    document_payload,
...
class DocumentEngine:
    ...
    :class:`~bumblebee.config.OcrConfig`, passed per call or as the engine-wide
    ``run_config`` default. Any config field left unset falls back to its
    ``BUMBLEBEE_*`` environment variable, then the built-in default.
    """
