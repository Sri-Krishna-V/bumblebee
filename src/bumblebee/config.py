...
* :class:`EngineConfig` — startup-scoped knobs (what gets loaded onto the GPU
  and how); passed to :class:`~bumblebee.engine.DocumentEngine` at construction.

Every field resolves the same way: an explicit value (CLI flag or constructor
argument) wins, else the field's ``BUMBLEBEE_*`` environment variable, else the
...
def config_env(config: Any) -> dict[str, str]:
    """Serialize a config's values back to their ``BUMBLEBEE_*`` variables.
...
class OcrConfig:
    ...
    force: bool = env_field("BUMBLEBEE_FORCE", False, parse_bool)
    ...
    pdf_dpi: int = env_field("BUMBLEBEE_PDF_DPI", 100, int)
    pdf_max_side: int = env_field("BUMBLEBEE_PDF_MAX_SIDE", 3500, int)
    page_chunk_size: int = env_field("BUMBLEBEE_PAGE_CHUNK_SIZE", 8, int)
    ...
    max_inflight_pdfs: int = env_field("BUMBLEBEE_MAX_INFLIGHT_PDFS", 16, int)
    ocr_request_concurrency: int = env_field("BUMBLEBEE_OCR_REQUEST_CONCURRENCY", 1024, int)
    storage_check_workers: int = env_field("BUMBLEBEE_STORAGE_CHECK_WORKERS", 64, int)
...
@dataclass(frozen=True, slots=True)
class EngineConfig:
    """Startup-scoped configuration for one :class:`~bumblebee.engine.DocumentEngine`.
    ...
    vllm_port: int = env_field("BUMBLEBEE_VLLM_PORT", 8000, int)
    vllm_health_timeout_seconds: int = env_field("BUMBLEBEE_VLLM_HEALTH_TIMEOUT_SECONDS", 15 * 60, int)
    ...
    gpu_memory_utilization: float = env_field("BUMBLEBEE_GPU_MEMORY_UTILIZATION", 0.60, float)
    max_model_len: int = env_field("BUMBLEBEE_MAX_MODEL_LEN", 8192, int)
    max_num_seqs: int = env_field("BUMBLEBEE_MAX_NUM_SEQS", 1024, int)
    max_num_batched_tokens: int = env_field("BUMBLEBEE_MAX_NUM_BATCHED_TOKENS", 16384, int)
    # API server scale-out parallelizes the CPU-bound front-end (tokenization +
    # multimodal preprocessing) and only pays off with spare CPU cores. 4 is
    # tuned for the default 16-core container; 8 on the same 16 cores
    # oversubscribed the CPU and tanked throughput (2.2 vs 8.0 pages/s). Scale it
    # with your vCPUs. (>1 also disables vLLM's scheduler stats logging.)
    api_server_count: int = env_field("BUMBLEBEE_API_SERVER_COUNT", 4, int)
    # Speculative decoding (e.g. GLM MTP): a JSON string passed to vLLM's
    # --speculative-config. An explicit empty string disables it.
    speculative_config: str = env_field(
        "BUMBLEBEE_SPECULATIVE_CONFIG",
        '{"method": "mtp", "num_speculative_tokens": 3}',
        str,
        empty_is_unset=False,
    )
    vllm_extra_args: str = env_field("BUMBLEBEE_VLLM_EXTRA_ARGS", "", str)
    ...
    layout_backend: LayoutBackend = env_field("BUMBLEBEE_LAYOUT_BACKEND", "onnx", str)
    ...
    # to match this batch.
    layout_batch_size: int = env_field("BUMBLEBEE_LAYOUT_BATCH_SIZE", 4, int)
    ...
    layout_threshold: float | None = env_field("BUMBLEBEE_LAYOUT_THRESHOLD", None, float)
    trt_layout_cache: str = env_field("BUMBLEBEE_TRT_LAYOUT_CACHE", "/root/.cache/vllm/trt_layout", str)
    trt_builder_opt_level: int = env_field("BUMBLEBEE_TRT_BUILDER_OPT_LEVEL", 1, int)

    # Thread-pool size for crop/resize/JPEG preparation, shared by all documents.
    crop_encode_workers: int = env_field("BUMBLEBEE_CROP_ENCODE_WORKERS", 8, int)
    ...
    def to_env(self) -> dict[str, str]:
        """Serialize resolved values back to their ``BUMBLEBEE_*`` variables."""
...
class ModalConfig:
    ...
    Consumed by the ``@app.cls`` decorators in :mod:`bumblebee.modal` at import
    time; bare-metal users choose these at provisioning time instead. The
    ``bumblebee modal`` CLI forwards explicit flags to the ``modal run``
    subprocess as environment variables.
    """

    app_name: str = env_field("BUMBLEBEE_APP_NAME", "bumblebee", str)
    gpu: str = env_field("BUMBLEBEE_GPU", "A100-40GB", str)
    cpu_cores: float = env_field("BUMBLEBEE_CPU_CORES", 16.0, float)
    cpu_limit: float | None = env_field("BUMBLEBEE_CPU_LIMIT", None, float)
    scaledown_window_seconds: int = env_field("BUMBLEBEE_SCALEDOWN_WINDOW_SECONDS", 15 * 60, int)
    startup_timeout_seconds: int = env_field("BUMBLEBEE_STARTUP_TIMEOUT_SECONDS", 20 * 60, int)
    timeout_seconds: int = env_field("BUMBLEBEE_TIMEOUT_SECONDS", 6 * 60 * 60, int)
    max_containers: int = env_field("BUMBLEBEE_MAX_CONTAINERS", 1, int)
    # Comma-separated Modal secret names mounted into the workers.
    secrets: str = env_field("BUMBLEBEE_MODAL_SECRET_NAMES", "", str)
