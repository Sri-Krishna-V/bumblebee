...
def _build_detector(config: EngineConfig) -> _Detector:
    if config.layout_backend == "onnx":
        try:
            from bumblebee.layout.onnx import OnnxLayoutDetector, tensorrt_providers
    ...
    from bumblebee.layout.torch import LayoutDetector
...
class LayoutEngine:
    ...
    The layout fields of :class:`~bumblebee.config.EngineConfig` are
