...
The heavy engine objects are imported lazily so ``import bumblebee`` (and reading
...
def __getattr__(name: str) -> object:
    ...
    :mod:`bumblebee.engine`), so it is deferred to first access (PEP 562).
    This keeps ``import bumblebee`` and reading :class:`OcrConfig` working
